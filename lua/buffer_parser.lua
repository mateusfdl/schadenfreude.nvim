...
function BufferParser:parse_buffer_content(content)
	...
	while i <= #lines do
		...
		if ai_begin_match then
			...
			-- Collect AI response content until @AI :FINISH
			while i <= #lines do
...
function BufferParser:extract_new_user_message(buffer)
	...
	-- Return the last message if it's from the user
...
function BufferParser:format_messages_for_api(messages, interface)
	local formatted = {}
